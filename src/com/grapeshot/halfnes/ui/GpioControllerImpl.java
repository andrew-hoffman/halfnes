...
class GpioControllerImpl {
    ...
    private static final int MENU = BIT8;
    private static final int MAGIC_MENU = START | SELECT;
    private static final int MAGIC_MENU_DECOMPOSE = MAGIC_START | MAGIC_SELECT;
        ...
        for (int i = 0; i < BUTTON_PINS.length; i++) {
            ...
            buttons[bx].addListener((GpioPinListenerDigital) event -> {
                if (event.getState().isLow()) { // press
                    ...
                    if ((gpiobyte & MAGIC_MENU) == MAGIC_MENU) {
                        gpiobyte &= ~MAGIC_MENU;
                        gpiobyte |= MENU;
                    }
                } else { // release
                    if ((gpiobyte & MENU) != 0 && (BUTTON_BIT[bx] & MAGIC_MENU_DECOMPOSE) != 0) {
                        gpiobyte &= ~MENU;
                        gpiobyte |= MAGIC_MENU;
                    }
